...
/// Fig. 7 driver: compute the contact curve over the worked-example grid and
/// write `target/fig7_contact.csv`.
///
/// Ref: \[KD20\] Fig. 7.
#[cfg(feature = "figures")]
pub fn fig7<D: Dynamics, C: CostModel>(dynamics: &D, cost: &C) {
    let grid = TimeGrid::uniform(T_I, T_F, GRID_DT).expect("valid grid");
    let w = sample_pseudostates(1, SEED)[0];
    let sol = solve(dynamics, cost, w, grid, &SolveParams::default()).expect("fig7 solve");
    let mut wtr = csv::Writer::from_path("target/fig7_contact.csv").expect("open fig7 csv");
    wtr.write_record(["t_s", "g"]).expect("header");
    for t in grid.times() {
        let y = dynamics.gamma(t).unwrap().transpose() * sol.lambda;
        let g = cost.at(t).contact(y);
        wtr.write_record(&[t.to_string(), g.to_string()])
            .expect("row");
    }
    wtr.flush().expect("flush");
    println!(
        "  Fig. 7 contact curve : target/fig7_contact.csv ({} rows)",
        grid.len()
    );
}
...
/// Ref: \[KD20\] Fig. 9.
#[cfg(feature = "figures")]
pub fn plot_fig9_timing(path: &str, rows: &[Fig9Row]) -> Result<(), Box<dyn std::error::Error>> {
    let root = BitMapBackend::new(path, (900, 600)).into_drawing_area();
    root.fill(&WHITE)?;
